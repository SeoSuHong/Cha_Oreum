...
function sendComment(no, nickname) {
	...
	$.ajax({
		type : "post",
		url : "comment?no=" + no + "&nickname=" + nickname + "&contents=" + contents.val() + "&secret=" + secret,
...
// 답글 입력 버튼 클릭 시
function sendReply(obj, no, comment_no, nickname) {
	...
	$.ajax({
		type : "post",
		url : "reply?no=" + no + "&comment_no=" + comment_no + "&nickname=" + nickname + "&contents=" + contents.val() + "&secret=" + secret,
		success : function(result) {
			if(result) {
				location.reload();
			} else {
...
function delete_notice(no) {
	let check = confirm("공지사항을 삭제 하시겠습니까?\n삭제된 게시글은 복구할 수 없습니다.");
